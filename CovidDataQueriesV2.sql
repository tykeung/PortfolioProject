...
Select Deaths.continent, Deaths.location, Deaths.date, Deaths.population, Total_Cases, Total_Deaths, 
Vaccine.New_Vaccinations, Total_tests, positive_rate, total_vaccinations, hospital_beds_per_thousand,
Vaccine.life_expectancy, Max(total_cases)/population as InfectionRate, Max(total_deaths)/population as DeathPercentage,
Max(total_vaccinations)/population as VaccinationRate
--Sum(Cast(Vaccine.new_vaccinations as float)) over (partition by Deaths.location order by Deaths.location, 
--Deaths.Date) as TotalVaccinationPerCountry 
...
where Deaths.continent is not null
and DeathRate < CONVERT(float, 1)
group by Deaths.continent, Deaths.location, Deaths.date, Deaths.population, Total_cases, total_deaths,
Vaccine.new_vaccinations, total_tests, positive_rate, total_vaccinations, hospital_beds_per_thousand,
